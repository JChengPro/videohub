...
<template>
  <main class="page profile-page">
    ...
    <section v-else-if="user" class="hero">
      ...
      <button v-if="user.id !== auth.claims?.account_id" type="button" :class="{ followed }" :disabled="followBusy" @click="toggleFollow">{{ followed ? '已关注' : '关注' }}</button>
    </section>
...
.hero { padding: 34px 18px 28px; display: grid; justify-items: center; background: linear-gradient(180deg, rgba(255,59,92,.08), transparent); }.hero h1 { margin-top: 13px; font-size: 21px; letter-spacing: -.03em; }.hero p { margin-top: 4px; color: var(--mobile-text-muted); font-size: 10px; }.hero > button { min-width: 120px; min-height: 44px; margin-top: 18px; padding: 10px 22px; border-radius: 999px; background: var(--mobile-accent); color: #fff; font-weight: 800; }.hero > button.followed { border: 1px solid var(--mobile-border); background: var(--mobile-surface-strong); color: var(--mobile-text-secondary); }
