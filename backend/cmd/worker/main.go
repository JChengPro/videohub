...
func main() {
	...
	log.Printf("worker storage initialized, type=%s", cfg.Storage.Type)
	...
	videoWorker := worker.NewVideoWorker(redisClient, fileStorage)
...
func consumeNotification(rabbit *mq.RabbitMQ, notificationWorker *worker.NotificationWorker) {
	...
	for d := range deliveries {
		...
		if err := json.Unmarshal(d.Body, &event); err != nil {
			log.Printf("invalid notification message: %s", string(d.Body))
			d.Nack(false, false)
			continue
		}
		if err := notificationWorker.Handle(context.Background(), event); err != nil {
			log.Printf("handle notification failed: %v", err)
			d.Nack(false, true)
			continue
		}
		if err := d.Ack(false); err != nil {
			log.Printf("ack notification message failed: %v", err)
		}
	}
}
